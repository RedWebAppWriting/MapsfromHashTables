 ...
 * </pre>
 *
 * @authors Sam Fess, Akhilesh Reddy
 *
 */
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        //get hashValue in order to determine the correct bin to store value
        int hashValue = key.hashCode();
        //increment size accordingly
        this.size++;
        int bin = mod(hashValue, this.hashTable.length);

        //Check to make sure that the bin does not already contain key
        if (!this.hashTable[bin].hasKey(key)) {
    ...
    public final Pair<K, V> remove(K key) {
        ...
        int hashValue = key.hashCode();
        //get hashValue in order to get to the correct bin and then remove the
        //key, adjust size accordingly
        int bin = mod(hashValue, this.hashTable.length);
        Map.Pair<K, V> removal = this.hashTable[bin].remove(key);
        this.size--;
    ...
    public final Pair<K, V> removeAny() {
        ...
        //set iterator to 0 and flag to false
        int binerator = 0;
        boolean fullBin = false;

        while (this.hashTable[binerator].size() == 0) {

            binerator++;

        }

        Map.Pair<K, V> removal = this.hashTable[binerator].removeAny();
        this.size--;
    ...
    public final V value(K key) {
        ...
        //generate the bin number from a key,
        //take out the corresponding pair from the bin
        //report the value
        //place values back into bin
        int hashValue = key.hashCode();
        int bin = mod(hashValue, this.hashTable.length);
        Map.Pair<K, V> removal = this.hashTable[bin].remove(key);
        V worth = removal.value();
        this.hashTable[bin].add(key, worth);
        return worth;
    }
    ...
    public final boolean hasKey(K key) {
        ...
        //given the key find the corresponding bin it would be kept into
        //through the hashing formula, then check that bin to see
        //if it is located there
        boolean checker = false;
        int hashValue = key.hashCode();
        int bin = mod(hashValue, this.hashTable.length);
        checker = this.hashTable[bin].hasKey(key);
        return checker;
    ...
    private final class Map4Iterator implements Iterator<Pair<K, V>> {
        ...
        public Pair<K, V> next() {
            ...
            if (!this.hasNext()) {
                /*
                 * Exception is supposed to be thrown in this case, but
